...
X = imread('Widy_5000us_NUC_694.tif');
X = flipud(X);
img_original = uint8(rescale(X,0,255));
            ...
            case 4
                for kk=1:length(intMethodStr)
                    ...
                    if l4plot
                        ...
                        savefig(fullfile('out',fname));
                        
                        img_rec1d = imresize(img_rec1,1/alpha,intMethodStr{kk});
                        img_rec2d = imresize(img_rec2,1/alpha,intMethodStr{kk});
                        img_rec3d = imresize(img_rec3,1/alpha,intMethodStr{kk});
                        ...
                        fname = ['dlevel1to4_' waveletStr{ii} '_' intMethodStr{kk} '.fig'];
                        savefig(fullfile('out',fname));
                        close all;
                    end
                end
