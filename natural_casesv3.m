        ...
        switch iterations{kk}
            ...
            case 1
                for jj=1:length(methodStr)
                    ...
                    img = imresize(img_original, alpha/2, methodStr{jj});

                    % Reconstruction
                    img_rec = idwt2(img,jLH,jHL,jHH,waveletStr{ii},size(img_original).*alpha).*alpha;
                    
                    %img_rec = imresize(img_rec, 1/alpha, methodStr{jj});
            ...
            case 2
                for jj=1:length(methodStr)
                    ...
                    % Interpolation
                    jHL = imresize(LLHL, alpha, methodStr{jj});
                    jLH = imresize(LLLH, alpha, methodStr{jj});
                    jHH = imresize(LLHH, alpha, methodStr{jj});
                    img = imresize(img_rec, alpha/2, methodStr{jj});
...
subplot(2,2,3:4); imagesc(img_reconstructed); colormap(gray); xlim([-100 size(img_reconstructed,2)+100]);
...
img_reconstructed = imresize(img_reconstructed, 1/2, intStr);
